...
- Vraie liste de raccourcis câblée dans `index.html` (5 raccourcis) : Musique → Apple Music, Dessiner → Tayasui Sketches, Créer → Incredibox (créa musicale simple, remplace l'ancien "Jeu éducatif / Prodigy Math"), Brain → lien direct vers l'app déployée (`https://web-steel-mu-60.vercel.app/`, pas un deep link natif), Roblox → déjà installée sur l'iPad (raccourci statique, aucune logique de gating/tracking ajoutée). Schemes iOS `tayasui-sketches://` et `incredibox://` non documentés publiquement, posés en meilleur effort — à vérifier lors du test sur iPad réel (`openShortcut` bascule de toute façon vers l'App Store après 1.2s si le scheme ne prend pas la main).
