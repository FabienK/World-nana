...
- Cadrage : `.scene` recadre `portal-full.jpg` (1000×666) au format 1000×556 via `aspect-ratio` + `object-fit: cover; object-position: center bottom` — seul le haut de l'image (le dessus de l'armoire) est rogné, sans zoom, donc la fillette garde sa taille d'origine et ses pieds restent visibles (un 1er essai au zoom coupait les pieds — retour testeur iPad du 14/09). Les % `top` du trou noir/ancre d'orbite/fillette ont été recalculés pour la nouvelle hauteur (les % `left`/largeurs n'ont pas bougé, seule la hauteur visible change).
